...
# Поддерживаем оба варианта расположения файла: words.csv в корне или data/words.csv
...
def make_options(word, direction: str):
    """Возвращает список вариантов [(text, is_correct), ...]."""
    field = "russian" if direction == "el_ru" else "greek"
    correct = word[field]

    with db() as conn:
        wrong_rows = conn.execute(
            f"SELECT {field} AS value FROM words WHERE id != ? ORDER BY RANDOM() LIMIT 3",
            (word["id"],)
        ).fetchall()
    ...
    # Если слов пока мало, просто показываем сколько есть вариантов.
...
async def start(message: Message):
    await message.answer(
        ...
        "Нажми «📚 Учить слова», и я начну спрашивать карточки с выбором 1 из 4.",
...
async def answer_button(callback: CallbackQuery):
    ...
    # Спрашиваем следующее слово автоматически
...
async def answer_check(message: Message):
    await message.answer(
        ...
        "Нажми «📚 Учить слова», чтобы получить вопрос.",
        reply_markup=main_keyboard()
    )
